...
<div class="pt-1 p-4 bg-slate-800">
    @if ($title !== 'Seluruh Pengaduan')
    <a href="/admin/seluruh-pengaduan" class="btn btn-sm w-full mb-5">
        << Kembali</a>
            @endif
                            ...
                            <div class="flex flex-col sm:flex-row gap-3">
                                <div>
                                    ...
                                    <select class="w-full sm:w-auto rounded-md p-1 pr-10 text-[15px]" name="bulan" id="bulan">
                                        ...
                                        @for ($m = 1; $m <= 12; $m++) <option value="{{ $m }}" {{ Session::get('bulan') == $m ? 'selected' : '' }}>
                            ...
                            <table class="table table-auto table-zebra table-compact w-full bg-white overflow-auto mb-5">
                                ...
                                <tbody>
                                    ...
                                    <tr class="[&>td]:border [&>td]:p-2 text-center">
                                        <th>
                                            <p class="">{{ $pengaduan->firstItem() + $index }}</p>
                                        </th>
                                        <td>
                                            <p class="">{{ date('d-m-Y', strtotime($i->tgl_pengaduan)) }}</p>
                                        </td>
                                        <td> <a href="https://wa.me/+62{{ $i->no_hp }}?text=Salam Sehat! Hello, kami dari Admin e-Lapor Sehati akan menanggapi laporan Anda '{{ $i->judul }}'." target="_blank" class="w-full btn btn-sm bg-gray-500 hover:bg-green-600">
                                        ...
                                        <td>
                                            <p class=" text-sm {{ $status->colorStatus($i) }} rounded-md p-1 mx-auto text-white w-32"><span class="">{{ $status->textStatus($i) }}</span></p>
                                        </td>
                                        <td class="">
                                    ...
                                    @else
                                    <td colspan="6">
                                        <p class="text-center text-2xl font-semibold p-5 rounded-b-md bg-slate-700 text-white">- Tidak ada Pengaduan -</p>
                                    </td>
                                    @endif
                                </tbody>
                            </table>
